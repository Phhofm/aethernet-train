...
class AetherNet(nn.Module):
    ...
    def prepare_qat(self, per_channel: bool = False):
        ...
        # Exclude all depthwise convolutions and custom modules
        layers_to_float = [
            'conv_first', 'conv_last', 'conv_before_upsample.0',
            'stages.*.conv',  # Skip depthwise convs
            'ffn.conv_.*',    # Skip FFN convolutions
            'channel_attn.*', # Skip channel attention
            'spatial_attn.*'  # Skip spatial attention
        ]
        
        # Also exclude by module type
        for name, module in self.named_modules():
            if isinstance(module, (ReparamLargeKernelConv, DynamicChannelScaling, 
                                SpatialAttention, GatedConvFFN)):
                module.qconfig = None
    ...
    def stabilize_fp16(self):
        """Apply comprehensive FP16 stabilization techniques"""
        # Stabilize normalization layers
        for module in self.modules():
            if isinstance(module, DeploymentNorm):
                # Clamp parameters to prevent underflow
                module.running_var.data.clamp_(min=1e-5)
                module.weight.data.clamp_(min=1e-5)
                module.bias.data.clamp_(min=-10, max=10)
                
            # Stabilize attention and gating parameters
            if hasattr(module, 'temperature'):
                module.temperature.data.clamp_(min=0.01, max=100)
            if hasattr(module, 'res_scale'):
                module.res_scale = max(module.res_scale, 0.01)
        
        # Stabilize convolutional weights
        for module in self.modules():
            if isinstance(module, nn.Conv2d):
                # Scale weights to prevent underflow
                weight_scale = 1.0 / module.weight.abs().max().clamp(min=1e-5)
                module.weight.data.mul_(weight_scale)
