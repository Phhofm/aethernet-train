...
class ReparamLargeKernelConv(nn.Module):
    ...
    def fuse(self):
        ...
        if self.is_quantized and hasattr(self.lk_conv, 'qconfig'):
            ...
            # Convert to quantized conv if needed
            if hasattr(self, 'weight_fake_quant'):
                # Create quantized convolution
                qconv = nn.quantized.Conv2d(
                    self.in_channels, self.out_channels, self.kernel_size,
                    self.stride, 0, groups=self.groups, bias=True
                )
                qconv.set_weight_bias(
                    self.weight_fake_quant(fused_kernel),
                    fused_bias
                )
                self.fused_conv = qconv
...
class DeploymentNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.training:
            # Calculate batch statistics - avoid in-place operations
            mean = x.mean(dim=(0, 2, 3), keepdim=True)
            var = x.var(dim=(0, 2, 3), keepdim=True, unbiased=False)
            # Update running stats with EMA without in-place operations
            new_mean = self.running_mean * 0.9 + mean * 0.1
            new_var = self.running_var * 0.9 + var * 0.1
            self.running_mean.copy_(new_mean)
            self.running_var.copy_(new_var)
            stats_mean, stats_var = mean, var
        else:
            stats_mean, stats_var = self.running_mean, self.running_var

        # Normalize and affine transform
        x = (x - stats_mean) / torch.sqrt(stats_var + self.eps)
        return x * self.weight + self.bias
...
class aether(nn.Module):
    ...
    def prepare_qat(self):
        ...
        This version correctly handles modern PyTorch APIs while forcing
        per-tensor quantization for weights to work around known bugs in some
        PyTorch versions.
        """
        if any(isinstance(m, LayerNorm2d) for m in self.modules()):
            print("Warning: QAT with LayerNorm may affect stability")

        print("INFO: Configuring QAT with PER-TENSOR weight quantization for maximum compatibility.")

        
        # --- START OF THE DEFINITIVE FIX ---
        try:
            # For modern PyTorch (2.0+), we build a custom QConfig.
            # We start with the modern defaults...
            from torch.ao.quantization import default_fake_quant
            from torch.ao.quantization.observer import MovingAverageMinMaxObserver
                    
            # Force per-tensor weight quantization
            ...
            # Fallback for very old PyTorch versions if the above fails.
            print("      - WARNING: Using legacy QAT configuration. This may not be as accurate or stable.")
    ...
    def convert_to_quantized(self) -> nn.Module:
        ...
        # Recursively remove any remaining fake quantization modules
        def remove_fake_quant(module):
            for name, child in module.named_children():
                if isinstance(child, tq.FakeQuantize):
                    setattr(module, name, nn.Identity())
                else:
                    remove_fake_quant(child)
        
        remove_fake_quant(quantized_model)
    ...
    def verify_quantization(self):
        ...
        if non_quantized:
            print("⚠️ Non-quantized layers detected:")
            for name in non_quantized:
                print(f"  - {name}")
        else:
            print("✅ All layers are quantized")
...
def export_onnx(
    ...
    # Use opset 18 for all exports
    opset = 18
    
    # Handle PyTorch version differences for training mode
    if hasattr(torch.onnx, 'TrainingMode'):
        training_mode = torch.onnx.TrainingMode.EVAL
    else:
        training_mode = False  # Fallback for older PyTorch versions

    # For INT8 models, ensure fake quant modules are removed
    if precision == 'int8':
        # Add final cleanup for quantization
        for module in model.modules():
            if hasattr(module, 'activation_post_process'):
                del module.activation_post_process

    # Add quantization checks
    if precision == 'int8' and hasattr(model, 'verify_quantization'):
        model.verify_quantization()
